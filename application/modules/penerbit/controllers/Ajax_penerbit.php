...
class Ajax_penerbit extends MY_Controller
{
    ...
    private function listdatahtml_dana() {
        $Request = $this->penerbit->HtmlDana($this->req);
        return $Request;
    }
    private function simpan_reset_password() {
        $Request = $this->penerbit->SimpanResetPassword($this->form);
        return $Request;
    }
    private function insert_lupa_password() {
        $Request = $this->penerbit->InsertLinkResetPassword($this->form);
        return $Request;
    }
    ...
    private function login() {
        $Request = $this->penerbit->LoginProcess($this->capt, $this->form);
        return $Request;
    }
