<?php
	...
	class Properti extends CI_Model {
			...
			if(!empty($query["data"])) {
				foreach ($query["data"] as $item) {
                    ...
                    if($item->status_projek == "Selesai"){
                    	$status_projek = "<br><br><label class='bold'><i class='fa fa-calendar-check'></i> ".$item->status_projek."</label><br><a href='".base_url("/properti/edit.html?id=").$item->id."' class='btn btn-primary m-1'>Perbarui</a><a class='btn btn-primary btn-kelola-dana text-white m-1' onclick='KelolaDana(".$item->id.");'>Kelola Dana</a>";
                    }
                    if($item->status_projek == "Tersedia"){
                    	$status_projek = "<br><br><a href='".base_url("/properti/edit.html?id=").$item->id."' class='btn btn-primary m-1'>Perbarui</a><a class='btn btn-primary btn-kelola-dana text-white m-1' onclick='KelolaDana(".$item->id.");'>Kelola Dana</a>";
                    }
		...
		public function InsertProperti($args) {
			...
			$args["id_penerbit"] = $this->user->id;
			$args["created_at"] = date("Y-m-d H:i:s");
			$args["created_by"] = $this->user->nama_perusahaan;
			$args["updated_at"] = date("Y-m-d H:i:s");
			$args["updated_by"] = $this->user->nama_perusahaan;
			...
			if(!$query){
				...
				foreach ($properti_dokumen as $item) {
	            	array_push($data_properti_dokumen, array(
						...
						"created_by"		=> $this->user->nama_perusahaan,
						"updated_by"		=> $this->user->nama_perusahaan
                    ));
                }
        ...
        public function UpdateProperti($id_update, $param) {
			...
			if($param["jumlah_dana"] == "" || $param["harga_per_lembar"] == "" || $param["total_per_lembar"] == "" || $param["roi"] == "" || $param["lepas_saham"] == "" || $param["nama"] == "" || $param["latitude"] == "" || $param["longitude"] == "" || $param["alamat"] == "" ){ 
				//|| $param["id_lokasi"] == "" $param["dividen_period"] == "" ||
			   	$return_data["IsError"] = true;
				$return_data["ErrorMessage"] = "Semua form wajib diisi";
				return json_encode($return_data);
			}
			...
			$param["id_penerbit"] = $this->user->id;
			$param["created_at"] = date("Y-m-d H:i:s");
			$param["created_by"] = $this->user->nama_perusahaan;
			$param["updated_at"] = date("Y-m-d H:i:s");
			$param["updated_by"] = $this->user->nama_perusahaan;
        ...
        public function InsertKelolaDana($args) {
			$args["no_kelola_dana"] = "KLD".substr(date("Y"), -2).date("md").rand(100000,999999);
			$args["created_at"] = date("Y-m-d H:i:s");
			$query = $this->db->insert("tb_kelola_dana", $args);
			if(!$query){
			   	$return_data["IsError"] = true;
				$return_data["ErrorMessage"] = $this->db->conn_id->error_list;
				return json_encode($return_data);
